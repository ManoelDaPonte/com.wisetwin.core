...
/// <summary>
/// Helper script pour s'assurer que les objets 3D correspondent aux IDs des metadata
/// Attachez ce script aux objets interactifs
/// </summary>
public class ObjectNameMapper : MonoBehaviour
{
    ...
    public string MetadataId => string.IsNullOrEmpty(metadataId) ? GenerateIdFromName() : metadataId;
    ...
    void Start()
    {
        if (autoDetectId && string.IsNullOrEmpty(metadataId))
        {
            ...
            Debug.Log($"[ObjectNameMapper] Auto-generated ID for {gameObject.name}: {metadataId}");
        }
        ...
        if (GetComponent<Collider>() == null)
        {
            ...
            Debug.Log($"[ObjectNameMapper] Added collider to {gameObject.name}");
        }
    }
    ...
    {
        Debug.Log($"[ObjectNameMapper] {gameObject.name} -> Metadata ID: {MetadataId}");
    }
}